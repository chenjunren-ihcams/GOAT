rm(list=ls())
library(readxl)
setwd('E:/work/project/GOAT/code/ws-agvhd-cdss/src/')
...
present.odds <- function(stationary.data.list,
  ...
  keys <- setdiff(keys1, keys1[grep('vitalsigns_[0-9]{3}', keys1)])#########Delete nursing records
  ...
  for (t in 1:100) {
    ...
    for (k in 1:length(keys)) {
      ...
      if (!is.na(val) & !is.na(thresholds.high.234[t, k])) {
        val <- as.numeric(val)
        if (val > thresholds.high.234[t, k]) {
          dynamic.odds.234[t] <- dynamic.odds.234[t] + log(odds.ratio.high.234[t, k])
        }
      }
      if (!is.na(val) & !is.na(thresholds.low.234[t, k])) {
        val <- as.numeric(val)
        if (val < thresholds.low.234[t, k]) {
          dynamic.odds.234[t] <- dynamic.odds.234[t] + log(odds.ratio.low.234[t, k])
        }
      }
      if (!is.na(val) & !is.na(thresholds.low.234[t, k]) & !is.na(thresholds.high.234[t, k])) {
        val <- as.numeric(val)
        if (val >= thresholds.low.234[t, k] & val <= thresholds.high.234[t, k]) {
          dynamic.odds.234[t] <- dynamic.odds.234[t] + log(odds.ratio.medium.234[t, k])
        }
      }
    }
  }
  ...
  for (t in 1: 100) {
    ...
    for (k in 1:length(keys)) {
      key <- keys[k]
      val <- dynamic[t, names(dynamic) == key]
      if (!is.na(val) & !is.na(thresholds.high.34[t, k])) {
        val <- as.numeric(val)
        if (val > thresholds.high.34[t, k]) {
          dynamic.odds.34[t] <- dynamic.odds.34[t] + log(odds.ratio.high.34[t, k])
        }
      }
      if (!is.na(val) & !is.na(thresholds.low.34[t, k])) {
        val <- as.numeric(val)
        if (val < thresholds.low.34[t, k]) {
          dynamic.odds.34[t] <- dynamic.odds.34[t] + log(odds.ratio.low.34[t, k])
        }
      }
      if (!is.na(val) & !is.na(thresholds.low.34[t, k]) & !is.na(thresholds.high.34[t, k])) {
        val <- as.numeric(val)
        if (val >= thresholds.low.34[t, k] & val <= thresholds.high.34[t, k]) {
          dynamic.odds.34[t] <- dynamic.odds.34[t] + log(odds.ratio.medium.34[t, k])
        }
      }
    }
  }
